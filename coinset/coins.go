...
func (cs *CoinSet) TotalValue() (value vdsutil.Amount) {
...
func satisfiesTargetValue(targetValue, minChange, totalValue vdsutil.Amount) bool {
...
type CoinSelector interface {
	CoinSelect(targetValue vdsutil.Amount, coins []Coin) (Coins, error)
}
...
type MinIndexCoinSelector struct {
	MaxInputs       int
	MinChangeAmount vdsutil.Amount
}

// CoinSelect will attempt to select coins using the algorithm described
// in the MinIndexCoinSelector struct.
func (s MinIndexCoinSelector) CoinSelect(targetValue vdsutil.Amount, coins []Coin) (Coins, error) {
	cs := NewCoinSet(nil)
...
func (s MinNumberCoinSelector) CoinSelect(targetValue vdsutil.Amount, coins []Coin) (Coins, error) {
...
func (s MaxValueAgeCoinSelector) CoinSelect(targetValue vdsutil.Amount, coins []Coin) (Coins, error) {
	sortedCoins := make([]Coin, 0, len(coins))
	sortedCoins = append(sortedCoins, coins...)
...
func (s MinPriorityCoinSelector) CoinSelect(targetValue vdsutil.Amount, coins []Coin) (Coins, error) {
...
// vdsutil.Tx, a specific outpoint index, and the number of confirmations
...
func (c *SimpleCoin) Value() vdsutil.Amount {
	return vdsutil.Amount(c.txOut().Value)
}
