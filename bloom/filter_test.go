...
import (
	...
	"github.com/merkleroot/vdsutil/bloom"
)
...
func TestFilterInsertKey(t *testing.T) {
	...
	wif, err := vdsutil.DecodeWIF(secret)
	if err != nil {
	...
	f.Add(vdsutil.Hash160(wif.SerializePubKey()))
...
func TestFilterBloomMatch(t *testing.T) {
	...
	tx, err := vdsutil.NewTxFromBytes(strBytes)
	if err != nil {
	...
	spendingTx, err := vdsutil.NewTxFromBytes(spendingTxBytes)
	if err != nil {
		t.Errorf("TestFilterBloomMatch NewTxFromBytes failure: %v", err)
		return
	}
...
func TestFilterInsertP2PubKeyOnly(t *testing.T) {
	...
	block, err := vdsutil.NewBlockFromBytes(blockBytes)
	if err != nil {
